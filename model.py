...
def log_prob_of_pair(p_matrix, current_id, next_id):
    ...
    ele = index_element(p_matrix,current_id,next_id)

    return array_log(ele)
...
def derive_dlogits_on_paper():
    ...
    return (
        "Let z be the logits and p = softmax(z). "
        "The mean cross-entropy loss is "
        "L = -(1/B) * sum(log(p_target)). "
        "Using the derivative of softmax and applying the chain rule, "
...
def train_neural_bigram_loop(w, data, block_size, batch_size, learning_rate, num_steps, log_every):
    ...
    for i in range(num_steps):
        ...
        ids = X.reshape(-1)
        target = y.reshape(-1)
        result = run_one_training_step(w, ids, target, learning_rate)
        w = result["w"]
        loss = result["loss"]
        if i%log_every==0:
            log.append(loss)
...
def layernorm_forward_variance(x, mean):
    ...
    return np.var(x,keepdims=True,axis=-1)
...
def attention_value_backward(d_attn_out, cache):
    ...
    d_v = attn_out.transpose(0,2,1) @ d_attn_out
